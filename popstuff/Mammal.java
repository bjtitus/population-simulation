...
public class Mammal extends Organism{
    ...
    public Mammal(String name, boolean meat, boolean plants, int x, int y, Grid<Structure> gr) {
		setName(name);
		double size = getDistNumber(50);
        setSize(size);
		setSpeed(getDistNumber(70));
		eatsMeat(meat);
		eatsPlants(plants);
		setLife(getDistNumber(150));
		setTemp(getDistNumber(10), getDistNumber(50));
	...
	public void act()
	{
		...
		{
			//ArrayList<Organism> organisms = getOrganisms();
	...
	public void move()
    {
        ...
        Location next = loc.getAdjacentLocation(getRandDirection());
        if (gr.isValid(next))
            moveTo(next);
        else
            removeSelfFromGrid();
